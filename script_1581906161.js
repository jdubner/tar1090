...
function refreshSelected() {
    ...
    if (selected.registration) {
        if (flightawareLinks) {
            ...
            $('#selected_registration').html(getFAAIdentLink(selected.registration, selected.registration));
        }
    } else {
...
// provide link to FAA registration database (only works on N-number registered aircraft)
function getFAAIdentLink(ident, linkText) {
    if (ident !== null && ident !== "") {
        if (!linkText) {
            linkText = ident;
        }
        return "<a target=\"_blank\" href=\"https://registry.faa.gov/aircraftinquiry/NNum_Results.aspx?Nnumbertxt=" + ident.trim() + "\">" + linkText + "</a>";
    }
